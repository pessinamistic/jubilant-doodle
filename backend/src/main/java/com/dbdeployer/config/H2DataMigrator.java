 ...
 *
 * <p>Only active when {@code app.migrate-from-h2=true}. Reads every row from the five live tables
 * in the source H2 file and inserts them into the already-migrated Postgres schema via the primary
 * {@link JdbcTemplate}.
 *
 * <p>Idempotent: each target table is skipped if it already contains rows.
 *
 * <p><b>Usage</b> — run the app once with:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>After verifying that all data landed in Postgres:
 *
 * <ol>
 *   <li>Delete this file
 *   <li>Remove {@code runtimeOnly("com.h2database:h2")} from {@code build.gradle.kts}
 * </ol>
 */
@Component
...
public class H2DataMigrator implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    log.info(
        "H2DataMigrator — migration complete. "
  ...
  private void migrateDeploymentConfig(JdbcTemplate h2) {
    if (targetHasRows("deployment_config")) return;
    ...
    for (Map<String, Object> r : rows) {
      pgJdbc.update(
          ...
          str(r, "VERSION"),
          intVal(r, "HOST_PORT"),
          intVal(r, "CONTAINER_PORT"),
          str(r, "USERNAME"),
          str(r, "PASSWORD"),
          str(r, "DATABASE_NAME"),
          str(r, "EXTRA_ENV_JSON"),
          str(r, "DEPLOY_METHOD"),
          boolVal(r, "IS_SYSTEM"),
          boolVal(r, "IS_IMPORTED"),
          ts(r, "CREATED_AT"),
          ts(r, "UPDATED_AT"));
    }
  ...
  private void migrateDeployedContainer(JdbcTemplate h2) {
    if (targetHasRows("deployed_container")) return;
    ...
    for (Map<String, Object> r : rows) {
      pgJdbc.update(
          ...
          str(r, "STATUS"),
          str(r, "DATA_DIRECTORY"),
          ts(r, "STARTED_AT"),
          ts(r, "REMOVED_AT"),
          str(r, "LATEST_PIPELINE_ID"),
          ts(r, "CREATED_AT"),
          ts(r, "UPDATED_AT"));
    }
  ...
  private void migrateDeploymentPipeline(JdbcTemplate h2) {
    if (targetHasRows("deployment_pipeline")) return;
  ...
  private void migratePipelineStep(JdbcTemplate h2) {
    if (targetHasRows("pipeline_step")) return;
  ...
  private void migrateImageTrackingStatus(JdbcTemplate h2) {
    if (targetHasRows("image_tracking_status")) return;
    ...
    for (Map<String, Object> r : rows) {
      pgJdbc.update(
          ...
          str(r, "IMAGE_NAME"),
          str(r, "IMAGE_TAG"),
          boolVal(r, "DOCKER_HUB_MANAGED"),
          str(r, "LOCAL_STATUS"),
          str(r, "DOCKER_HUB_STATUS"),
          str(r, "DECISION"),
          str(r, "MESSAGE"),
          ts(r, "LOCAL_CHECKED_AT"),
          ts(r, "DOCKER_HUB_CHECKED_AT"),
          ts(r, "CREATED_AT"),
          ts(r, "UPDATED_AT"));
    }
  ...
  private static Timestamp ts(Map<String, Object> row, String col) {
    ...
    if (v instanceof java.time.Instant i) return Timestamp.from(i);
    return null;
  }
}
