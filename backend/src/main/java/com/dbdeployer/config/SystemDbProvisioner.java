...
public class SystemDbProvisioner
  ...
  public void initialize(ConfigurableApplicationContext ctx) {
    ...
    boolean autoProvision =
        Boolean.parseBoolean(env.getProperty("dbdeployer.system-db.auto-provision", "true"));
    if (!autoProvision) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Docker is not available at "
              + dockerHost
              + ". Port Wrangler requires Docker to manage "
  ...
  private void waitForPostgres(int port, int maxSeconds) {
    ...
    for (int elapsedSeconds = 0; elapsedSeconds < safeMaxSeconds; elapsedSeconds++) {
      ...
      log.info(
          "Postgres startup progress {}",
          formatProgressBar(elapsedSeconds + 1, safeMaxSeconds, false));
    ...
    throw new IllegalStateException(
        "System Postgres did not become ready within "
            + maxSeconds
            + " seconds on port "
            + port
