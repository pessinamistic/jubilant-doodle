...
/**
 * Checks whether the Docker daemon is reachable on the current machine by
 * performing a real {@code ping} against the socket resolved by
 * {@link DockerSocketResolver}.  This is deliberately separate from
 * {@link com.dbdeployer.deploy.OsDetector#isDockerAvailable()}, which only
 * tests whether the {@code docker} CLI binary is on the PATH — not whether
...
public class DockerHealthChecker {
    ...
    /**
     * Attempts to ping the Docker daemon.  Never throws — all failures are
     * captured in the returned {@link DockerStatus}.
     */
