...
public class SystemDbRegistrar implements ApplicationRunner {
    ...
    public SystemDbRegistrar(
            DeploymentConfigRepository configRepo, DeployedContainerRepository containerRepo, JdbcTemplate jdbc) {
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            DeployedContainer container =
                    containerRepo.findByConfigId(SYSTEM_CONFIG_ID).orElse(null);
            boolean changed = false;
            if (container == null) {
    ...
    private String resolveRuntimeContainerId() {
        String fromSystemProperty = System.getProperty(SystemDbProvisioner.RUNTIME_CONTAINER_ID_PROPERTY, "")
                .trim();
    ...
    private String resolveRuntimeContainerName() {
        String fromSystemProperty = System.getProperty(SystemDbProvisioner.RUNTIME_CONTAINER_NAME_PROPERTY, "")
                .trim();
        if (!fromSystemProperty.isBlank()) return fromSystemProperty;
