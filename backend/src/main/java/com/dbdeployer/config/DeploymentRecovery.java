 ...
 *
 * <ol>
 *   <li><b>Containers stuck in DEPLOYING</b> — app crashed before async deploy finished. If {@code
 *       containerId == null}: mark ERROR. Otherwise, ask Docker for ground truth.
 *   <li><b>Pipelines stuck in RUNNING</b> — app crashed while a pipeline was executing. Mark the
...
public class DeploymentRecovery implements ApplicationRunner {
  ...
  private void recoverContainers() {
    List<DeployedContainer> stuck =
        deployedContainerRepository.findByStatus(InstanceStatus.DEPLOYING);
  ...
  private void recoverPipelines() {
    List<DeploymentPipeline> stuck =
        deploymentPipelineRepository.findByStatus(PipelineStatus.RUNNING);
    if (stuck.isEmpty()) return;

    log.info(
        "DeploymentRecovery: {} pipeline(s) stuck in RUNNING — marking FAILED...", stuck.size());
    ...
    for (DeploymentPipeline pipeline : stuck) {
      ...
      pipelineStepRepository
          .findByPipelineIdOrderByStepOrderAsc(pipeline.getId())
          .forEach(
              step -> {
