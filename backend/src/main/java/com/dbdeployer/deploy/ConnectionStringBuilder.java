...
public class ConnectionStringBuilder {
  ...
  public String build(
    ...
    String password = coalesce(config.getPassword(), firstPlaceholder(def, DatabaseCatalog.EnvVarType.PASSWORD), "");
    String database = coalesce(config.getDatabaseName(), firstPlaceholder(def, DatabaseCatalog.EnvVarType.DATABASE),
        "");

    return def.connectionStringTemplate().replace("{username}", username).replace("{password}", password)
        .replace("{port}", String.valueOf(config.getHostPort())).replace("{database}", database);
  }
  ...
  @SafeVarargs
  private static <T extends CharSequence> String coalesce(
    T... candidates) {
    for (T c : candidates) {
