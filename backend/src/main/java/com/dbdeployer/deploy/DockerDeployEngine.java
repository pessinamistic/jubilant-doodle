...
public class DockerDeployEngine {
    ...
    public InstanceStatus getStatus(DeployedContainer container) {
        ...
        try {
            InspectContainerResponse info =
                    docker.inspectContainerCmd(container.getContainerId()).exec();
    ...
    public Instant getStartedAt(String containerId) {
        try {
            String raw =
                    docker.inspectContainerCmd(containerId).exec().getState().getStartedAt();
    ...
    public List<DiscoveredContainerDto> discoverContainers(Set<String> trackedIds, Set<String> trackedNames) {
        ...
        for (Container c : running) {
            ...
            result.add(new DiscoveredContainerDto(
                    c.getId(), name, c.getImage(), dbType, displayName, icon, hostPort, containerPort, c.getState()));
        }
    ...
    private Set<String> localRefCandidates(String image, String tag) {
        ...
        boolean hasRegistryHost =
                parts.length > 0 && (parts[0].contains(".") || parts[0].contains(":") || "localhost".equals(parts[0]));
                    ...
                    switch (ev.type()) {
                        ...
                        case DATABASE ->
                            config.getDatabaseName() != null
                                            && !config.getDatabaseName().isBlank()
