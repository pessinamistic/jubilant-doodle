...
public interface DeployedContainerRepository extends JpaRepository<DeployedContainer, String> {
  ...
  Optional<DeployedContainer> findFirstByConfigIdAndStatusNotOrderByCreatedAtDesc(
      String configId, InstanceStatus status);

  /**
   * Returns true if any non-removed instance is already using this port. REMOVED configs do not
   * block port reuse.
   */
  @Query(
      "SELECT CASE WHEN COUNT(dc) > 0 THEN true ELSE false END FROM DeployedContainer dc "
          + "WHERE dc.hostPort = :hostPort AND "
          + "(dc.status <> com.dbdeployer.model.InstanceStatus.REMOVED)")
