...
public class ContainerCreateStep implements DeployStep {
    ...
    public String execute(DeploymentConfig config, DeployedContainer container) throws StepExecutionException {
        ...
        try {
            ...
            throw new StepExecutionException(
                    DeployErrorCode.VOLUME_CREATE_FAILED, "Failed to create data directory: " + e.getMessage(), e);
        } catch (Exception e) {
            throw new StepExecutionException(
                    DeployErrorCode.CONTAINER_CREATE_FAILED, "Failed to create container: " + e.getMessage(), e);
        }
