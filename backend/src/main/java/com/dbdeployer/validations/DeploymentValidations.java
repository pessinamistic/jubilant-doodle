...
public class DeploymentValidations {
  ...
  public DeploymentValidations(
      ImageValidationService imageValidation, DeployedContainerRepository containerRepo) {
    this.imageValidation = imageValidation;
    this.containerRepo = containerRepo;
  }

  // @Override
  public void validate(ConfigTemplateRequest deployRequest, boolean isTemplate) {
    ...
    if (containerRepo.existsByContainerName(deployRequest.name())) {
      ...
      throw new IllegalArgumentException(
          "An instance named '" + deployRequest.name() + "' already exists");
    }

    if (containerRepo.existsByHostPortAndNotRemoved(deployRequest.hostPort())) {
      log.error(
          "[deploy] Rejecting request: host port {} already in use", deployRequest.hostPort());
    ...
    var imageCheck =
        imageValidation.checkForDeploy(deployRequest.dbType(), deployRequest.version());
    log.info(
        "[deploy] Image check result for {}:{} -> decision={}, local={}, hub={}, message='{}'",
        def.dockerImage(),
        deployRequest.version(),
        imageCheck.decision(),
        imageCheck.localStatus(),
        imageCheck.dockerHubStatus(),
        imageCheck.message());
    if (imageCheck.decision() == ImageValidationDecision.BLOCK) {
      log.warn(
          "[deploy] Blocking deployment '{}' because image is not deployable: {}",
          deployRequest.name(),
          imageCheck.message());
    ...
    if (imageCheck.decision() == ImageValidationDecision.ALLOW_WITH_WARNING) {
      log.warn(
          "Proceeding with deployment '{}' despite warning: {}",
          deployRequest.name(),
          imageCheck.message());
    }
  }
